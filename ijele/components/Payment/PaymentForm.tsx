...
const PaymentForm = () => {
  ...
  const handlePaymentSubmit = async (e: any) => {
    ...
    if (amounts[currentParty] !== contributions[currentParty]) {
      alert(`Party ${currentParty + 1}'s contribution must be ${contributions[currentParty]}.`);
      return;
    }
          ...
          <h1>Page in Testing Status - Please do not enter actual payment info!!!</h1>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleAmountChange(currentParty, e.target.value)}
                className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                required
              />
            </div>
            <div>
              ...
              >
                {currentParty < parties - 1 ? "Pay & Next Party" : "Pay & Finish"}
              </button>
            </div>
          </form>
        </div>
